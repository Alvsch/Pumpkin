...
use pumpkin_world_core::{
    BlockEntity, BlockFlags, Clearable, Inventory, InventoryFuture, PropertyDelegate, SimpleWorld,
};
...
impl BrewingStandBlockEntity {
    ...
    async fn do_brew(&self, world: &Arc<dyn SimpleWorld>, ingredient: &ItemStack) {
        let ingredient_id = ingredient.get_item().id;
...
impl Inventory for BrewingStandBlockEntity {
    fn size(&self) -> usize {
        Self::INVENTORY_SIZE
    }
...
impl Clearable for BrewingStandBlockEntity {
