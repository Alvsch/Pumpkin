...
use pumpkin_world_core::{BlockEntity, Clearable, Inventory, PropertyDelegate};
...
macro_rules! impl_cooking_block_entity_base {
    ($struct_name:ty) => {
        impl $crate::entities::furnace_like_block_entity::CookingBlockEntityBase for $struct_name {
            fn get_cooking_time_spent(&self) -> u16 {
            ...
            async fn craft_recipe(
                ...
                use pumpkin_world_core::Inventory as _;
...
macro_rules! impl_property_delegate_for_cooking {
    ($struct_name:ty) => {
        use crate::entities::furnace_like_block_entity::CookingBlockEntityBase as _;

        impl pumpkin_world_core::PropertyDelegate for $struct_name {
...
macro_rules! impl_clearable_for_cooking {
    ($struct_name:ty) => {
        impl pumpkin_world_core::Clearable for $struct_name {
...
macro_rules! impl_experience_container_for_cooking {
    ($struct_name:ty) => {
        impl pumpkin_world_core::ExperienceContainer for $struct_name {
...
macro_rules! impl_inventory_for_cooking {
    ($struct_name:ty) => {
        impl pumpkin_world_core::Inventory for $struct_name {
            fn size(&self) -> usize {
                self.items.len()
            }

            fn is_empty(&self) -> pumpkin_world_core::InventoryFuture<'_, bool> {
                Box::pin(async move {
                    for slot in self.items.iter() {
            ...
            fn get_stack(
                &self,
                slot: usize,
            ) -> pumpkin_world_core::InventoryFuture<'_, Arc<Mutex<ItemStack>>> {
            ...
            fn remove_stack(
                &self,
                slot: usize,
            ) -> pumpkin_world_core::InventoryFuture<'_, ItemStack> {
                Box::pin(async move {
            ...
            fn remove_stack_specific(
                ...
                Box::pin(async move {
                    let res = pumpkin_world_core::split_stack(&self.items, slot, amount).await;
                    self.mark_dirty();
                    res
                })
            }
            ...
            fn set_stack(
                ...
                slot: usize,
                stack: ItemStack,
            ) -> pumpkin_world_core::InventoryFuture<'_, ()> {
                Box::pin(async move {
...
macro_rules! impl_block_entity_for_cooking {
    ($struct_name:ty,$recipe_kind:expr) => {
        impl pumpkin_world_core::BlockEntity for $struct_name {
            fn tick<'a>(
                &'a self,
                world: &'a Arc<dyn pumpkin_world_core::SimpleWorld>,
                        ...
                        if self.is_burning() {
                            ...
                            world
                                .set_block_state(
                                    ...
                                    pumpkin_world_core::BlockFlags::NOTIFY_ALL,
                                )
                                .await;
                        } else {
                            props.lit = false;
            ...
            fn get_inventory(self: Arc<Self>) -> Option<Arc<dyn pumpkin_world_core::Inventory>> {
                Some(self)
            }
            ...
            fn to_property_delegate(
                self: Arc<Self>,
            ) -> Option<Arc<dyn pumpkin_world_core::PropertyDelegate>> {
                Some(self as Arc<dyn pumpkin_world_core::PropertyDelegate>)
            }

            fn to_experience_container(
                self: Arc<Self>,
            ) -> Option<Arc<dyn pumpkin_world_core::ExperienceContainer>> {
                Some(self as Arc<dyn pumpkin_world_core::ExperienceContainer>)
            }
        }
    };
}
