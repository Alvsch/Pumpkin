        ...
        impl pumpkin_world_core::BlockEntity for $struct_name {
                ...
                Self: Sized,
            {
                ...
                let chest = Self {
                    ...
                    viewers: $crate::viewer::ViewerCountTracker::new(),
                };
...
macro_rules! impl_inventory_for_chest {
    ($struct_name:ty) => {
        impl pumpkin_world_core::Inventory for $struct_name {
            ...
            fn on_open(&self) -> pumpkin_world_core::InventoryFuture<'_, ()> {
                Box::pin(async move {
                    self.viewers.open_container();
                })
            }

            fn on_close(&self) -> pumpkin_world_core::InventoryFuture<'_, ()> {
                Box::pin(async move {
        ...
        impl pumpkin_world_core::Clearable for $struct_name {
            fn clear(
                ...
                Box::pin(async move {
                    ...
                    <$struct_name as pumpkin_world_core::Inventory>::mark_dirty(self);
                })
            }
        }
    };
...
macro_rules! impl_viewer_count_listener_for_chest {
    ($struct_name:ty) => {
        impl $crate::viewer::ViewerCountListener for $struct_name {
            fn on_container_open<'a>(
                &'a self,
                world: &'a Arc<dyn pumpkin_world_core::SimpleWorld>,
                _position: &'a pumpkin_util::math::position::BlockPos,
            ) -> $crate::viewer::ViewerFuture<'a, ()> {
                Box::pin(async move {
...
macro_rules! impl_chest_helper_methods {
    ($struct_name:ty) => {
        impl $struct_name {
            ...
            async fn play_sound(
                &self,
                world: &Arc<dyn pumpkin_world_core::SimpleWorld>,
