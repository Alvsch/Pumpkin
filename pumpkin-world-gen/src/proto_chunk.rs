...
use crate::noise::aquifer_sampler::{FluidLevel, FluidLevelSamplerImpl};
use crate::noise::perlin::DoublePerlinNoiseSampler;
use crate::noise::router::multi_noise_sampler::MultiNoiseSamplerBuilderOptions;
use crate::noise::router::surface_height_sampler::SurfaceHeightSamplerBuilderOptions;
use crate::noise::{CHUNK_DIM, ChunkNoiseGenerator, LAVA_BLOCK, WATER_BLOCK};
use crate::structure::lazily_generate_structure;
use crate::structure::placement::should_generate_structure;
use crate::structure::structures::{
    StructureGeneratorContext, StructureInstance, create_chunk_random,
};
use crate::structure::try_generate_structure;
use crate::surface::rule::try_apply_material_rule;
use crate::{
    BlockStateId,
    {biome, positions::chunk_pos},
};
...
pub struct ProtoChunk {
    ...
    pub carving_mask: crate::carver::mask::CarvingMask,
...
impl ProtoChunk {
    ...
    pub fn new(x: i32, z: i32, generator: &super::generator::VanillaGenerator) -> Self {
        ...
        Self {
            ...
            carving_mask: crate::carver::mask::CarvingMask::new(height as i32, dimension.min_y),
                    ...
                    for rx in (region_x - 1)..=(region_x + 1) {
                        for rz in (region_z - 1)..=(region_z + 1) {
                            candidate_chunks.push(
                                crate::structure::placement::get_structure_chunk_in_region(
                                    spread,
                                    seed,
                                    rx,
                                    rz,
