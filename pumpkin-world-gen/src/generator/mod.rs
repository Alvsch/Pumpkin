...
use crate::proto_chunk::TerrainCache;
use crate::{GlobalRandomConfig, Seed};
...
pub struct VanillaGenerator {
    ...
    pub global_structure_cache: crate::structure::placement::GlobalStructureCache,
    ...
    fn new(seed: Seed, dimension: Dimension) -> Self {
        ...
        for (i, set) in StructureSet::ALL.iter().enumerate() {
            structure_allowed_biomes
                .insert(i, crate::proto_chunk::ProtoChunk::get_allowed_biomes(set));
        }
        ...
        Self {
            ...
            terrain_cache,
            default_block,
            global_structure_cache: crate::structure::placement::GlobalStructureCache::new(),
